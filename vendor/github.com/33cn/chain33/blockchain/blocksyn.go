	...
	for key, value := range chain.bestChainPeerList {
		...
		if value.IsBestChain {
			...
			if !ok {
				...
				Peeraddrs = append(Peeraddrs, value.Peer.Addr)
			}
		}
	}
	synlog.Debug("GetBestChainPids", "pids", PeerPids)
	go printLocationAPI(Peeraddrs)
	return PeerPids
}

//test
func printLocationAPI(peersaddr []string) {
	for _, addr := range peersaddr {
		ipinfo := LocationAPI(addr)
		if ipinfo != nil {
			if _, ok := ipinfo.Data.(map[string]interface{}); ok {
				synlog.Debug("printLocationAPI:map", "ipaddr", addr, "ipinfo", ipinfo.Data.(map[string]interface{}))
			} else {
				synlog.Debug("printLocationAPI", "ipaddr", addr, "ipinfo", ipinfo.Data.(string))
			}
		}
	}
}
